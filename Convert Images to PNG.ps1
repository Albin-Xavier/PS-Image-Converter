...
if (Test-Path -Path .\ImageMagick\mogrify.exe -Pathtype Leaf) {
	...
	.\ImageMagick\mogrify.exe -path .\Output -format png Input\*.webp Input\*.jpg Input\*.heic Input\*.tiff
	move-item -Force Input\*.webp, Input\*.jpg, Input\*.heic, Input\*.tiff -Destination .\Originals\
	}
}
